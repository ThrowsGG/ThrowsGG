...
public class categoryFilteringAction implements Action {
	private static final int LISTCOUNT = 6;
	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("categoryFilteringAction start");
		
		HttpSession session = request.getSession();
		String userid = (String) session.getAttribute("id");
		String[] checkArray=null;
		if(request.getParameterValues("cate")==null) {
			checkArray= new String[4];
		}else {
			checkArray = request.getParameterValues("cate");
		}
		...
		ArrayList<ProductVO> productList=instance.getCateFilterProduct(pageNum, limit, checkArray);
		
		int total_record = instance.countCateFilter(checkArray);
		int total_page;
...
//		forward.setPath("/shop.str");
		...
		System.out.println("categoryFilteringAction end");
