...
public class IndexCommentsDAO {
	...
	public ArrayList<IndexCommentsVO> selectComments(String desertionNo) {
		...
		String sql = "SELECT indc.indexComments" + " , indc.comment" + " , indc.desertionNo" + " , indc.userID"
				+ " , indc.regist_date" + " , indc.nickname" + " , mem.profile" + " FROM throwsgg.indexcomments AS indc"
				+ " INNER JOIN throwsgg.member AS mem" + " ON indc.userID = mem.id" + " where desertionNo = ?";
		ResultSet rs = null;
		try {
			// conn 생성
	...
	public void insertComments(String comments, String userID, String desertionNo, String nickname) {
		String sql = "INSERT INTO throwsgg.indexcomments (comment, desertionNo,userID,nickname) "
				+ "VALUES (?, ?, ?,?)";
		try {
			// conn 생성
			conn = ds.getConnection();
	...
	public void childInsertComments(String comments, String userID, String desertionNo, int groupNum, String nickname) {
		String sql = "INSERT INTO throwsgg.childcomments (comment, desertionNo,userID,groupNum,usernickname) "
				+ "VALUES (?, ?, ?,?,?)";
		try {
			// conn 생성
			conn = ds.getConnection();
	...
	public ArrayList<ChildIndexCommentsVO> childSelectComments(String desertionNo) {
		...
		String sql = "SELECT indc.childIndex" + " , indc.comment" + " , indc.desertionNo" + " , indc.userID"
				+ " , indc.regist_date" + " , indc.groupNum" + " , indc.usernickname" + " , mem.profile"
				+ " FROM throwsgg.childcomments AS indc" + " INNER JOIN throwsgg.member AS mem"
				+ " ON indc.userID = mem.id" + " where desertionNo = ?";
		ResultSet rs = null;
		try {
			// conn 생성
			conn = ds.getConnection();
	...
	public void childdeleteComments(int indexComments) {
		String sql = "DELETE FROM throwsgg.childcomments WHERE (childIndex = ?)";
		try {
			// conn 생성
			conn = ds.getConnection();
			// pstmt 생성
	...
	public void childModifyComments(String comments, String indexComments) {
		String sql = "UPDATE throwsgg.childcomments SET comment = ? WHERE (childIndex = ?)";
		try {
			// conn 생성
			conn = ds.getConnection();
			// pstmt 생성
	...
	public boolean checkReservation(String resdesertionNo) {
		String sql = "select exists(SELECT * FROM reservationtable "
				+ "WHERE resDate >= CURDATE() and resdesertionNo = ? ORDER BY resDate DESC)";
		ResultSet rs = null;
		...
		try {
			...
			// pstmt 생성
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, resdesertionNo);
			pstmt.executeQuery();
			rs = pstmt.executeQuery();
			while (rs.next()) {
				checkReserve = rs.getBoolean(1);
			}
			System.out.println(checkReserve);
